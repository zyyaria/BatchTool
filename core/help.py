...
def get_pdf_help_text():
    ...
    ● 颜色模式：彩色、灰度、黑白（灰度减约 30%，黑白减 70% 以上）<br>
...
def get_img_help_text():
    ...
    ● 图片质量：1~100，JPG / WEBP 控制画质（值越大图片越清晰），PNG 控制压缩级别（100=无压缩，1=极限压缩）<br>
    ...
    ● 保持比例：勾选保持比例，不勾选则拉伸<br>
    ● 目标分辨率：勾选修改分辨率可自定义分辨率<br><br>

    <b>5. 图片分切裁剪</b><br><br>
    ● 分切模式：分切（行数 / 列数）、裁剪（比例 / 尺寸）<br>
    ...
    ● 分组方式：按文件名前缀长度、每 N 个一组、按文件夹、所有文件<br>
    ● 速度 / 重复：10~5000 ms，控制每帧播放延时；0 = 无限循环<br>
    ● 目标尺寸：保持原尺寸、自定义，勾选保持比例避免变形<br>
    ● 排列方式：顺序播放（一个接一个播放）、同时叠加（多个 GIF 同时播放，拼在一个画面里）<br>
    ● 时间同步：按最短时长截断（短的播完就停，长的被截断）、按最长时长循环（长的播完才停，短的循环补齐）<br>
    ● 拼接方式：水平、垂直、网格（行数 / 列数）<br>
    ● 背景色 / 边距 / 间距：自定义背景色（HEX）、边距、间距<br>
