...
def get_img_help_text():
    ...
    ● 目标尺寸：像素、百分比、短边约束、长边约束<br>
    ● 保持比例：勾选保持比例，不勾选则拉伸<br>
    ● 目标 DPI：1~3000，修改打印密度<br>
    ● 输出格式：原格式、PNG、JPG、WEBP、BMP、TIFF、GIF、ICO<br><br>

    <b>3. 压缩图片文件</b><br><br>
    ● 预设：轻度、中等、强力、极限<br>
    ● 输出格式：原格式、JPG、PNG、WEBP、GIF<br>
    ...
    ● 布局模式：垂直、水平、网格（每行张数）、台词拼接（偏移量）<br>
    ● 偏移量：截图无黑边设为负数，有黑边设为 0 或正数<br>
    ● 间距 / 背景：间距 0~200 px，自定义背景色（HEX）<br>
    ● 输出格式：PNG、JPG、WEBP<br>
    ● 标签设置：勾选添加序号或添加文件名生效，显示在上方或下方，自定义高度和大小<br>
    ● 标题设置：标题不为空时生效，显示在顶部或底部，自定义高度和大小<br><br>

    <b>5. 图片分切裁剪</b><br><br>
    ● 分切模式：水平（上下切）、垂直（左右切）、网格（自定义行列数）<br><br>
...
def get_video_help_text():
    ...
    <br><b>1. 视频拼接合并</b><br><br>
    ● 分组方式：按文件名前缀长度、每 N 个一组、按文件夹、所有文件<br>
    ● 输出格式：mp4、mkv、avi、mov<br>
    ...
    ● 输出格式：原格式、mp4、mkv、avi、mov<br>
    ● 重新编码：重新编码兼容性更好<br><br>

    <br><b>3. 视频转GIF图片</b><br><br>    
