...
def get_img_help_text():
    ...
    <b>3. 多图合成 GIF</b><br><br>
    ● 分组方式：按文件名前缀长度、每 N 个一组、按文件夹、所有文件<br>
    ● 帧间隔：10~5000 ms<br>
    ● 循环次数：0=无限循环<br>
    ● 目标尺寸：保持原尺寸、自定义宽高<br><br>

    <b>4. 动图拼接合并</b><br><br>
    ● 分组方式：按文件名前缀长度、每 N 个一组、按文件夹、所有文件<br>
    ● 播放类型：顺序播放（一个接一个播放）、同时播放（多个 GIF 同时播放，拼在一个画面里）<br>
    ● 播放速度：10~5000 ms，控制每帧播放延时<br>
    ● 重复次数：-1=不重复，0=无限循环，>0=指定次数<br>
    ● 目标尺寸：保持原尺寸或自定义宽高（同时播放是指每张 GIF 的宽高），勾选「保持比例」避免变形<br>
    ● 拼接方式：水平、垂直、网格拼接（行列数）<br>
    ● 背景 / 边距 / 间距：自定义背景色（HEX）、边距、间距<br>
    ● 播放时长：按最短时长截断（短的播完就停，长的被截断）或按最长时长循环（长的播完才停，短的循环补齐）<br><br>    
    ...
    <b>7. 图片分切裁剪</b><br><br>
    ● 分切模式：水平（上下切）、垂直（左右切）、网格（自定义行列数）<br>
