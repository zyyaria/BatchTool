...
from PySide6.QtCore import (
    QEasingCurve, QObject, QPoint, QPropertyAnimation, Qt, 
    QThread, Signal
)
...
from .utils import NamingRules, resource_path, sanitize_base_name
from .help import GENERAL_HELP_TEXT, get_about_text
from .dialogs import NamingRulesDialog
...
class UIMixin:
    ...
    def _build_right_panel(self):
        ...
        self.grp_specific = QGroupBox("功能设置")
        ...
        self.btn_start.clicked.connect(lambda: self._on_start_clicked())
...
class BaseMainWindow(UIMixin, QMainWindow):
    ...
    def _run_batch_task(self, checked_items):
        ...
        def on_batch_finished(output_files):
            ...
            if elapsed >= 60:
                ...
                self.append_log(f"本次操作用时: {minutes}分{seconds}秒")
            else:
                self.append_log(f"本次操作用时: {elapsed:.1f} 秒")
    ...
    def _animate_button_click(self, btn):
        ...
        anim.setEndValue(original_pos + QPoint(0, 2))
        anim.setEasingCurve(QEasingCurve.OutQuad)
        anim.finished.connect(lambda: self._restore_button(btn, original_pos))
        anim.start()

    def _restore_button(self, btn, original_pos):
        """按钮弹起复位动画"""
        anim = QPropertyAnimation(btn, b"pos")
        anim.setDuration(80)
    ...
    def _on_start_clicked(self):
        """开始按钮点击处理：先播放动画再启动任务"""
        self._animate_button_click(self.btn_start)
        from PySide6.QtCore import QTimer
